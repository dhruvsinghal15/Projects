...
void tabu_search(int max_iter, float seed_A, float seed_B, float fseed);
...
int main()                                    // the main function
{
   ...
   printf("\nMaximum of the function C = -A^2-B^2 + 5 ");                         // introduction to the program
   ...
   seed_A=(PAR_MAX-PAR_MIN)*(((float)rand())/RAND_MAX)+PAR_MIN;
   //printf("\nseed=%f", seed_A);
   seed_B=(PAR_MAX-PAR_MIN)*(((float)rand())/RAND_MAX)+PAR_MIN;
   fseed=C(seed_A,seed_B);
    
   tabu_search(num, seed_A, seed_B, fseed);  
...
float C(float x, float y)          // the y function that we look for it's maximum value takes (x,y) value
{
   float t;
   t=-(x*x)-(y*y)+5;            // the function is C= - ( x^ 2 ) - (y^ 2) +5
   return(t);              
}                              // end of C function
...
void tabu_search(int max_iter, float seed_A, float seed_B, float fseed)
{
     ...
     best.point[1]=seed_B;
     best.fitness=fseed;
     printf("\n seed=(%f,%f) with fitness=%f", seed_A, seed_B, best.fitness);
...
void neighborhood(info curr_best, info curr_neighborhood[DIM_POP])  /* make a neighborhood of each point of the curr_neighborhood */
{
     ...
     while(S!=DIM_POP && M!=5*DIM_POP ) //the loop stops when S=DIM_POP or when it is reached a establish max number without has found neighborhoods
     {
      ...
      for(i=0;i<S;i++)
      {
         ...
         curr_neighborhood[i].fitness=C(curr_neighborhood[i].point[0], 
